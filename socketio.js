...
// serve socketio
server.listen(conf.socketio_port, {
	cors: {
...
io.use((socket, next) => {
	...
	if (socket.handshake.query && socket.handshake.query.token){
		request
		.get(get_url(socket, "/api/method/snd_ecommerce.chat.get_user_info_from_session"))
		.type("form")
		.set('Authorization', `token ${socket.handshake.query.token}`)
		.then((res) => {
			if (res && res.body && res.body.message && res.body.message.user && res.body.message.user != "Guest"){
